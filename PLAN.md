...
- 2026-08-22 (session 1j): Search mixin landed — search.rs (largest state
  ...
  (unit).documented divergence in the module). Batch 29 -
  `tools/mcp_dashboard_oauth.py` (11 tests, source-derived - gap noted:
  publish pins the URL's state param and moves starting to
  authorization_required; wait-for-URL wakes on publish or flow error;
  callback validates state with a constant-time compare, rejects
