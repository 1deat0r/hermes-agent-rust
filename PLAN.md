...
**P2 status: 🟡 OPEN — hermes-tools support wave and provider profile base/registry/Actual/AI Gateway/Alibaba/Alibaba Coding Plan/Anthropic/Arcee/Azure Foundry/Bedrock/Copilot/Copilot ACP/Custom/DeepInfra/DeepSeek/Fireworks/Gemini/GMI/Kilo/Kimi Coding/Minimax/Novita/NVIDIA/Nous/Ollama Cloud/OpenCode Go/OpenCode Zen/OpenAI Codex/Qwen OAuth/StepFun/Upstage/Vertex/XAI/Xiaomi/ZAI/Hugging Face profiles landed (2026-08-24).** The support wave below
...
Section 5: `_summarize_api_error` landed via `ApiErrorShape`
(settled `is_value_error`, pre-read `response.text`, status,
type-name): ValueError arm, HTML title/Ray collapse, Gemini
passthrough, body-dict coerce+decorate, #36109 response-text path
(redacted), decorated fallback. Oracles:
`test_summarize_api_error` (empty-body JSON, unreadable response),
`test_nonretryable_error_html_summary` (challenge collapse);
...
| plugins/model-providers/opencode-zen/__init__.py (147 LOC) | ✅ | `hermes-providers::profiles::opencode_zen` and the OpenCode Go request hook; 5 source-derived metadata/alias/model-normalization/reasoning/max-token parity tests (`unit`) cover Zen/Go metadata, aliases, Kimi/DeepSeek/GLM routing, disable/no-config behavior, mutually exclusive wire shapes, and the `mimo-v2.5-pro` cap; dynamic `api_mode` routing and CLI/transport integration remain future higher-layer seams |
...
- `plugins/model-providers/opencode-zen/__init__.py` + `tests/plugins/model_providers/test_opencode_go_profile.py` → `crates/hermes-providers/tests/parity_opencode_zen.rs` (5 source-derived metadata/alias/reasoning/max-token parity tests; `unit`; dynamic `api_mode` routing and CLI/transport integration remain future higher-layer oracles)
...
- `agent/ssl_verify.py` + `tests/agent/test_ssl_verify.py` → `crates/hermes-agent/tests/parity_auxiliary_client.rs` (4 TLS-resolver parity tests including both warning texts; `unit`/`mock`)
...
- 2026-09-16 (Wave A batch 3): remaining 21 platform `__init__` entries done
  (literals harvested per-adapter; a2a in-init register + teams computed-hint
...
- 2026-09-16 (Wave B timefmt/budget unit): `coerce_epoch` + rewire (live
  goldens), `normalize_budget_warning_ratio` (RatioInput seam); audio +
  debug_helpers recerts. Known-flaky: daemon_pool `max_workers` peak test
...
- 2026-08-31 (session 4da): Fifty units across four crates, all red-first:
  ...
  idle to 0.0. Batch 3 — `gateway/readiness.py` (9 tests): read-only URI
  state-db probe with a 1s busy timeout, empty YAML documents count as
  ...
  noted in the module doc). Batch 7 —
  `agent/trajectory.py` (7 tests, source-derived; gap noted —
  scratchpad→think tag conversion with the opening-tag pre-check,
  ...
  Batch 36 - `agent/secret_sources/base.py` (9 tests, source-derived -
  gap noted: env-name validation, ANSI scrubbing incl. unterminated OSC,
  the per-fetch environment view with token reset, fetch-timeout
...
- 2026-08-28 (session 4d5): Promoted `agent.ssl_verify` to done and recorded
  ...
  `hermes-agent::auxiliary_client::resolve_auxiliary_tls_verify` (the same
  hosting arrangement `agent/redact` uses in `hermes-logging`), but the port
  silently dropped both `logger.warning` calls and the `base_url` argument that
  only exists to fill the insecure-mode message. `resolve_httpx_verify`'s
  contract is now complete: the resolver keeps the same return shape, the new
  pure `auxiliary_tls_verify_resolution` returns `(verify, warning)`, and
  `AuxiliaryTlsVerifyWarning::text()` reproduces both messages byte-for-byte
  including the `"a custom provider endpoint"` placeholder; the resolver logs
  the same string through `log::warn!`. 4 TLS parity tests (`unit`/`mock`)
  cover it, with the resolver signature gaining the source's `base_url` input.
  Explicit non-port decision (needs sign-off, recorded here):
  `agent/jiter_preload.py` (39 LOC) will not be ported. Its entire content is a
  best-effort `importlib.import_module("jiter.jiter")` at package-import time to
  dodge a Windows CPython import-order failure inside the OpenAI SDK's streamed
  response path, plus its two module globals and the import-time call. Rust
  links the parser at build time — there is no deferred native-extension
  import, no `sys.modules` entry to pre-seed, and no equivalent failure mode —
  so a "port" would be an empty function that can never fail. The row therefore
  stays `missing` with this rationale rather than being padded with a no-op.
  Evidence: `/home/mustbearnold/.cargo/bin/cargo test -p hermes-agent --test
  parity_auxiliary_client` passes 62 tests; `/home/mustbearnold/.cargo/bin/cargo
  build --workspace` green; the required serialized
  `/home/mustbearnold/.cargo/bin/cargo test --workspace -- --test-threads=1`
  and `git diff --check` results are recorded below; ledger moves to 84 done /
  13 partial / 3,785 tracked (**2.16%**) and 84 done / 13 partial / 1,006
  production (**7.62%**).
...
- 2026-08-24 (session 4d0): Continued Phase 2 through two parallel
  ...
  gpt-5.4/5.5/5.6, and Codex Spark. Added OpenCode Zen and Go profiles,
  aliases, registry/loader wiring, Go Kimi/DeepSeek/GLM reasoning mappings,
  mutually-exclusive wire shapes, and the `mimo-v2.5-pro` 131,072 cap.
  Focused suites pass 46 auxiliary, 5 OpenCode, and 8 registry tests
  (`unit`); `cargo check` passes for both crates. Workspace validation:
  `/home/mustbearnold/.cargo/bin/cargo build --workspace` and
  `/home/mustbearnold/.cargo/bin/cargo test --workspace --
  --test-threads=1` pass 1,155 tests with 5 ignored and 20 warnings.
  Targeted rustfmt check and `git diff --check` pass. The currently scoped
  hermes-conversion gates are all met (36/36). No module-status or ledger
  change; generated inventory remains 73 done / 11 partial / 3,798 missing
  tracked and 73 done / 11 partial / 1,019 missing production modules:
  1.88% tracked strict completion and 6.62% production. The next
  dependency-safe unit is higher-layer provider/auth transport or
  auxiliary-client integration; full hermes-cli managed overlay/default-
  catalog wiring remains blocked by the absent hermes-cli crate.
