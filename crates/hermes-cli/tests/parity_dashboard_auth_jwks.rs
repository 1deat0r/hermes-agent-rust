...
fn transport_failure_is_provider_error() {
    let err = classify_jwks_lookup_error(JwksLookupFailure::Connection, "Fail to fetch data");
...
fn unverifiable_tokens_are_invalid_code() {
    for failure in [
        ...
        let err = classify_jwks_lookup_error(failure, "bad");
        assert!(matches!(err, JwksClassify::InvalidCode(_)), "{failure:?}");
    }
}
