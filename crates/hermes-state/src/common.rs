...
pub const _PREVIEW_CONTENT_SQL: &str = "REPLACE(REPLACE(m.content, X'0A', ' '), X'0D', ' ')";
...
mod tests {
    ...
    fn parity_with_upstream_golden_state_common() {
        ...
        let el: Vec<&str> = golden["escape_like"]
            .as_array()
            .unwrap()
            .iter()
            .map(|v| v.as_str().unwrap())
        ...
        assert_eq!(
            ...
            golden["preview_content_sql"].as_str().unwrap()
        );
        assert_eq!(
        ...
        assert_eq!(
            _preview_raw_select(),
            golden["preview_raw_select"].as_str().unwrap()
        );
    }
}
