...
fn record_and_get_active_cooldown() {
    ...
    let cd = db
        .get_compression_failure_cooldown("s1")
        .expect("get")
        .expect("some");
...
fn expired_cooldown_is_inactive_but_row_preserved() {
    ...
    assert_eq!(
        db.get_compression_failure_cooldown("s1").expect("get"),
        None
    );
...
fn cooldown_row_absent_and_missing_session() {
    ...
    let row = db
        .get_compression_failure_cooldown_row("nope")
        .expect("row");
    assert_eq!(
        row,
    ...
    assert_eq!(
        db.get_compression_failure_cooldown("nope").expect("get"),
        None
    );
...
fn restore_roundtrip_and_absent_session_guard() {
    ...
    db.restore_compression_failure_cooldown_row("s1", &snapshot)
        .expect("restore");
...
fn clear_cooldown_and_empty_id_noops() {
    ...
    assert_eq!(
        db.get_compression_failure_cooldown("").expect("empty"),
        None
    );
    assert_eq!(
    ...
    assert_eq!(
        db.get_compression_fallback_streak("")
            .expect("empty streak"),
        0
    );
    db.set_compression_fallback_streak("", 5)
        .expect("set empty");
    assert_eq!(
    ...
    db.set_compression_ineffective_count("", 5)
        .expect("set empty");
}
...
fn fallback_streak_roundtrip_and_clamp() {
    ...
    db.create_session("s1", "cli", &NewSession::default())
        .expect("create");
    assert_eq!(
        db.get_compression_fallback_streak("s1").expect("default"),
        0
    );
    // Missing session reads as 0.
    assert_eq!(
        db.get_compression_fallback_streak("nope").expect("missing"),
        0
    );
    ...
    db.set_compression_fallback_streak("s1", -2)
        .expect("set neg");
    assert_eq!(
        db.get_compression_fallback_streak("s1")
            .expect("get clamped"),
        0
    );
}
...
fn ineffective_count_roundtrip_and_clamp() {
    ...
    db.create_session("s1", "cli", &NewSession::default())
        .expect("create");
    assert_eq!(
        db.get_compression_ineffective_count("s1").expect("default"),
        0
    );

    db.set_compression_ineffective_count("s1", 4)
        .expect("set 4");
    assert_eq!(
        db.get_compression_ineffective_count("s1").expect("get 4"),
        4
    );
    db.set_compression_ineffective_count("s1", -1)
        .expect("set neg");
    assert_eq!(
        db.get_compression_ineffective_count("s1")
            .expect("get clamped"),
        0
    );
}
