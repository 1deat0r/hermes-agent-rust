...
fn search_finds_content_and_returns_context() {
    ...
    db.create_session("s1", "cli", &NewSession::default())
        .unwrap();
    db.append_message("s1", &msg("user", "How do I deploy with Docker?"), None)
        .unwrap();
    db.append_message("s1", &msg("assistant", "Use docker compose up."), None)
        .unwrap();

    let results = db
        .search_messages("docker", None, None, None, 20, 0, None, false, None)
        .unwrap();
    assert_eq!(results.len(), 2);
    let snippets: Vec<&str> = results
        .iter()
        .filter_map(|r| r.get("snippet").and_then(|v| v.as_str()))
        .collect();
    assert!(snippets.iter().any(|s| s.to_lowercase().contains("docker")));

    // Context is present by default and non-empty.
    db.append_message("s1", &msg("user", "Tell me about Kubernetes"), None)
        .unwrap();
    db.append_message(
        "s1",
        &msg("assistant", "Kubernetes is an orchestrator."),
        None,
    )
    .unwrap();
    let results = db
        .search_messages("Kubernetes", None, None, None, 20, 0, None, false, None)
        .unwrap();
    assert_eq!(results.len(), 2);
...
fn search_fields_project_results_without_changing_default() {
    ...
    assert!(default.iter().all(|r| r["context"]
        .as_array()
        .map(|a| !a.is_empty())
        .unwrap_or(false)));
    ...
    let err = db
        ...
        .unwrap_err();
    assert!(
        matches!(err, hermes_state::state::WriteError::ValueError(_)),
        "got {:?}",
        err
    );
    db.close();
...
fn sanitizer_matches_upstream_unit_cases() {
    ...
    assert_eq!(
        s("TODO: fix").split_whitespace().collect::<Vec<_>>(),
        vec!["TODO", "fix"]
    );
...
fn mixed_cjk_english_query_and_like_wildcards() {
    ...
    db.create_session("s1", "cli", &NewSession::default())
        .unwrap();
    db.append_message("s1", &msg("user", "讨论Agent通信协议"), None)
        .unwrap();
    let results = db
        .search_messages("Agent通信", None, None, None, 20, 0, None, false, None)
        .unwrap();
    ...
    db.create_session("s2", "cli", &NewSession::default())
        .unwrap();
    db.append_message("s1", &msg("user", "达成100%完成率"), None)
        .unwrap();
    db.append_message("s2", &msg("user", "达成100完成率是目标"), None)
        .unwrap();
    let results = db
        .search_messages("100%完成", None, None, None, 20, 0, None, false, None)
        .unwrap();
    assert_eq!(results.len(), 1);
...
fn search_messages_excludes_tool_source() {
    ...
    let (_dir, path) = tmp_db("state.db");
    let db = SessionDB::open(Some(path), false).expect("open");
    db.create_session("py", "cli", &NewSession::default())
        .unwrap();
    db.create_session("tool", "tool", &NewSession::default())
        .unwrap();
    db.append_message(
        "py",
        &msg("user", "Python is a language used in infra"),
        None,
    )
    .unwrap();
    db.append_message("tool", &msg("user", "Python tool run details"), None)
        .unwrap();

    let all = db
        .search_messages("Python", None, None, None, 20, 0, None, false, None)
        .unwrap();
...
fn search_messages_sees_tool_name_and_tool_calls() {
    ...
    let r1 = db
        .search_messages("UNIQUETOOLNAME", None, None, None, 20, 0, None, false, None)
        .unwrap();
    let r2 = db
        .search_messages(
            "UNIQUESEARCHTOKEN",
...
fn search_visibility_compacted_discoverable_rewound_hidden() {
    ...
    db.append_messages_batch(
        "s1",
        &[msg("user", "needle alpha"), msg("assistant", "reply alpha")],
        None,
        None,
    )
    .unwrap();
    db.archive_and_compact("s1", &[msg("user", "needle beta")], None)
        .unwrap();

    // Compacted-archived "needle alpha" rows ARE discoverable by default.
    let archived = db
        .search_messages("needle", None, None, None, 20, 0, None, false, None)
        .unwrap();
    ...
    let target: i64 = db
        .writer_conn()
        .query_row(
            "SELECT id FROM messages WHERE session_id='s1' AND active=1 LIMIT 1",
            [],
            |r| r.get(0),
        )
        .unwrap();
    ...
    let after = db
        ...
        .unwrap();
    assert!(
        after.iter().all(|m| m["id"].as_i64() != Some(target)),
        "rewound live row must be hidden"
    );
    // include_inactive=True finds it again.
    let all = db
        .search_messages("needle", None, None, None, 20, 0, None, true, None)
        .unwrap();
...
fn seed_long_session(db: &SessionDB, sid: &str, n: i64) -> Vec<i64> {
    ...
    for i in 0..n {
        ...
        let mid = db
            .append_message(sid, &msg(role, &format!("prose msg {}", i)), None)
            .unwrap();
        ids.push(mid);
    }
    ids
...
fn anchored_view_window_and_bookends() {
    ...
    let start_ids: Vec<i64> = view["bookend_start"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let end_ids: Vec<i64> = view["bookend_end"]
        .as_array()
        .unwrap()
        .iter()
        .map(|m| m["id"].as_i64().unwrap())
...
fn anchored_view_role_filter_and_anchor_preserved() {
    ...
    for i in 0..5 {
        user_ids.push(
            db.append_message("s1", &msg("user", &format!("u{}", i)), None)
                .unwrap(),
        );
        db.append_message(
            "s1",
    ...
    let tool_id = db
        .writer_conn()
        .query_row(
            "SELECT id FROM messages WHERE role='tool' AND session_id='s1' LIMIT 1",
            [],
            |r| r.get::<_, i64>(0),
        )
        .unwrap();
    ...
    let view = db
        .get_anchored_view("s1", user_ids[2], 5, 0, Some(&roles))
        .unwrap();
    let window_roles: Vec<&str> = view["window"]
        .as_array()
        .unwrap()
        .iter()
    ...
    let view = db
        .get_anchored_view("s1", tool_id, 5, 0, Some(&roles))
        .unwrap();
    let window_ids: Vec<i64> = view["window"]
        .as_array()
        .unwrap()
        .iter()
        .map(|m| m["id"].as_i64().unwrap())
...
fn recent_user_messages_excludes_bookkeeping_and_handoffs() {
    ...
    db.create_session(sid, "cli", &NewSession::default())
        .unwrap();
    db.append_message(sid, &msg("user", "real turn one"), None)
        .unwrap();
    db.append_message(sid, &msg("assistant", "a1"), None)
        .unwrap();
    ...
    assert!(recent
        .iter()
        .all(|r| r["preview"].as_str().unwrap() != "model_switch marker"));
    assert_eq!(recent[0]["preview"].as_str().unwrap().chars().count(), 80);
    assert_eq!(
        recent[0]["preview"].as_str().unwrap().chars().last(),
        Some('.')
    );
    ...
    assert!(recent2.iter().all(|r| !r["preview"]
        .as_str()
        .unwrap()
...
fn optimize_and_rebuild_are_safe_on_fresh_db() {
    ...
    db.create_session("s1", "cli", &NewSession::default())
        .unwrap();
    db.append_message("s1", &msg("user", "needle in the haystack"), None)
        .unwrap();
...
fn deferred_rebuild_chunk_loop_indexes_messages() {
    ...
    let results = db
        .search_messages("chunkneedle", None, None, None, 100, 0, None, false, None)
        .unwrap();
...
fn sort_newest_and_oldest_respected() {
    ...
    let oldest = db
        ...
        .unwrap();
    let newest_ts: Vec<f64> = newest
        .iter()
        .map(|r| r["timestamp"].as_f64().unwrap())
        .collect();
    let oldest_ts: Vec<f64> = oldest
        .iter()
        .map(|r| r["timestamp"].as_f64().unwrap())
        .collect();
