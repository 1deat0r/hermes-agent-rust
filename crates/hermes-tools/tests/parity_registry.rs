...
fn register_and_dispatch() {
    ...
    assert_eq!(
        result
            .as_str()
            .map(|s| serde_json::from_str::<Value>(s).expect("parse")),
        Some(json!({"ok": true}))
    );
}
...
fn plugin_override_gate_blocks_unowned_deregister() {
    ...
    t.register(
        ...
        None,
        None,
        Some("hermes_plugins.pkg.handlers".to_string()),
        false,
    )
    .expect("r");
    ...
    t.deregister("builtin", Some("hermes_plugins.pkg.cleanup"))
        .expect("owner okay");
    ...
    t.deregister("builtin2", Some("hermes_plugins.other.handlers"))
        .expect("opted in");
}
...
fn aliases_and_generation_surfaces() {
    ...
    assert_eq!(
        t.get_toolset_alias_target("mcp-alias").as_deref(),
        Some("canonical")
    );
    let aliases = t.get_registered_toolset_aliases();
    assert_eq!(
        aliases.get("mcp-alias").map(|s| s.as_str()),
        Some("canonical")
    );
}
