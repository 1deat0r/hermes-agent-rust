...
fn seed_modpack_sessions(db: &SessionDB) {
    let now_ts = now();
    db.create_session("s_oldest", "cli", &NewSession::default())
        .expect("oldest");
    ...
    db.append_message(
        "s_oldest",
        &msg("user", "Let's build a Minecraft modpack"),
        None,
    )
    .expect("m1");
    db.append_message(
        "s_oldest",
        &msg("assistant", "Great. Let me scaffold the modpack repo."),
        None,
    )
    .expect("m2");
    db.append_message("s_oldest", &msg("user", "Use NeoForge 1.21.1"), None)
        .expect("m3");
    db.append_message(
        "s_oldest",
        &msg(
            "assistant",
            "Done. Modpack repo created with NeoForge 1.21.1.",
        ),
        None,
    )
    .expect("m4");
    db.append_message(
        ...
        &msg(
            "assistant",
            "Tier-0 mods installed; modpack smoke test passes.",
        ),
        None,
    )
    .expect("m5");

    db.create_session("s_middle", "cli", &NewSession::default())
        .expect("middle");
    let conn = db.writer_conn();
    conn.execute(
        "UPDATE sessions SET started_at = ?, title = ? WHERE id = ?",
    ...
    db.append_message(
        "s_middle",
        &msg("user", "Deep-dive every modpack reference quest guide"),
        None,
    )
    .expect("n1");
    db.append_message(
        ...
        &msg(
            "assistant",
            "Surveying ATM10 questbook for modpack inspiration.",
        ),
        None,
    )
    .expect("n2");
    db.append_message(
        "s_middle",
        &msg("user", "Update the modpack version too"),
        None,
    )
    .expect("n3");
    db.append_message(
        ...
        &msg(
            "assistant",
            "Modpack version bumped 0.4 → 0.8.5; quest coverage page added.",
        ),
        None,
    )
    .expect("n4");

    db.create_session("s_newest", "cli", &NewSession::default())
        .expect("newest");
    let conn = db.writer_conn();
    conn.execute(
        "UPDATE sessions SET started_at = ?, title = ? WHERE id = ?",
    ...
    db.append_message(
        "s_newest",
        &msg("user", "Fix the modpack mob spawning"),
        None,
    )
    .expect("o1");
    db.append_message(
        ...
        &msg(
            "assistant",
            "Investigating elite mob gating in the modpack KubeJS.",
        ),
        None,
    )
    .expect("o2");
    db.append_message(
        ...
        &msg(
            "assistant",
            "Shipped commit b850442. Modpack alternator nerfed too.",
        ),
        None,
    )
    .expect("o3");
...
fn discovery_sort_newest_and_oldest() {
    let (_dir, db) = open_db("state.db");
    seed_modpack_sessions(&db);
    let newest = parse(&session_search(
        Some(&db),
        "modpack",
        None,
        3,
    ...
    let oldest = parse(&session_search(
        Some(&db),
        "modpack",
        None,
        3,
    ...
    assert_eq!(
        oldest["results"][0]["session_id"].as_str(),
        Some("s_oldest")
    );
}
...
fn scroll_returns_anchored_window_without_bookends() {
    ...
    let anchor_sid = disc["results"][0]["session_id"]
        .as_str()
        .unwrap()
        .to_string();
...
fn read_returns_full_session() {
    ...
    assert_eq!(
        result["session_meta"]["title"].as_str(),
        Some("Building the Modpack")
    );
}
...
fn schema_params_cover_every_shape() {
    ...
    let params = SESSION_SEARCH_SCHEMA["parameters"]["properties"]
        .as_object()
        .unwrap();
    assert!(params.contains_key("query"));
    assert!(params.contains_key("limit"));
    assert_eq!(
        SESSION_SEARCH_SCHEMA["parameters"]["properties"]["sort"]["enum"],
        json!(["newest", "oldest"])
    );
...
fn registry_registers_session_search() {
    ...
    let reg = registry();
    assert_eq!(
        reg.get_toolset_for_tool("session_search").as_deref(),
        Some("session_search")
    );
    assert_eq!(reg.get_emoji("session_search", "⚡"), "🔍");
    let defs = reg.get_definitions(
        &std::collections::HashSet::from(["session_search".to_string()]),
        false,
    );
